...
details = ScDetails()
client = soundcloud.Client(client_id=details.client_id, client_secret=details.client_secret, username=details.username,
...
def get_others_favourites():
    # if current file exists and is less than a day old
    if os.path.isfile('data/others_favourites.txt') and os.path.getmtime('data/others_favourites.txt') > (
        time.time() - 86400):
